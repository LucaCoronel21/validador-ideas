...
const PUBLIC_PATHS = ["/login", "/auth/callback", "/api/jobs", "/api/dev-test"];
...
export async function proxy(request: NextRequest) {
  ...
  // servidor de Supabase en vez de solo leer la cookie, así el chequeo de
  // "protegido" no confía en un token que ya pudo haber sido revocado.
  const {
    data: { user },
